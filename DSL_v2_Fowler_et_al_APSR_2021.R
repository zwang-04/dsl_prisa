...
# ========= Prepare Human Data for Sampling =========
...
cat("Human data prepared for sampling:", nrow(cand_ads_human), "\n")
...
for (bootstrap_iter in 1:100) {
  ...
  for (tone in tone_vars) {
    ...
    # 使用 ad_content_human sample
    base_h <- as.data.table(ad_content_human)
    
    # 可用于标注的基础行
    eligible_idx <- which(!is.na(base_h[[tone_human_col]]))
    available_labeled <- length(eligible_idx)
    cat("Available labeled base rows:", available_labeled, "\n")
    if (available_labeled < sample_size) {
      cat("警告: 可用基础样本数", available_labeled, "小于请求的样本数", sample_size, "，跳过\n")
      next
    }
    
    # 基础层抽样
    set.seed(123 + bootstrap_iter + sample_size * 1000)
    sampled_idx <- sample(eligible_idx, sample_size)
    base_h$is_labeled <- 0L
    base_h$is_labeled[sampled_idx] <- 1L
    
    # 基础层均匀抽样概率 basic_sample_prob
    p_basic <- sample_size / available_labeled
    base_h$basic_sample_prob <- 0
    base_h$basic_sample_prob[eligible_idx] <- p_basic
    
    # 基础层生成被标注的值
    base_h[[tone_labeled_col]] <- ifelse(base_h$is_labeled == 1L, base_h[[tone_human_col]], NA)
    
    # 抽样结果映射到候选人×平台
    sampled_map <- merge(
      base_h[, .(original_row_id, is_labeled, basic_sample_prob, 
                 tone_labeled = get(tone_labeled_col))],
      as.data.table(cand_ads_human)[!is.na(spend), .(original_row_id, cand_id, fb, spend)],
      by = "original_row_id", all.x = FALSE
    )
    
    # LLM 侧的候选人×平台聚合
    unit_llm <- cand_ad_avg_llm[, .(cand_id, fb, v_llm = get(tone_llm_col))]
    
    # human 被标注的候选人×平台聚合
    unit_labeled <- sampled_map[is_labeled == 1L, .(
      v_labeled = if (all(is.na(tone_labeled))) NA_real_ else weighted.mean(tone_labeled, spend, na.rm = TRUE)
    ), by = .(cand_id, fb)]
    
    # 聚合抽样概率
    unit_prob <- sampled_map[, .(
      agg_sample_prob = 1 - prod(1 - basic_sample_prob)
    ), by = .(cand_id, fb)]
    
    current_df <- merge(unit_llm, unit_labeled, by = c("cand_id", "fb"), all.x = TRUE)
    current_df <- merge(current_df, unit_prob, by = c("cand_id", "fb"), all.x = TRUE)
    
    setDT(current_df)
    setnames(current_df, c("v_llm", "v_labeled"), c(tone_llm_col, tone_labeled_col))
    ...
    # 计算basic_n_labeled
    basic_n_labeled <- sum(base_h$is_labeled, na.rm = TRUE)
